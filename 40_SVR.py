...
print(f"在测试集上的均方误差mse：{mse:.2f}")
print(f"在测试集上的均方根误差rmse：{rmse:.2f}")
print(f"在测试集上的平均绝对误差mae：{mae:.2f}")
print(f"在测试集上的R²得分r2：{r2:.2f}")

# 新数据点
new_data = np.array([[0.5, 0.5, 200, 40]])
...
plt.title('Importance of feature(40μm dataset)')
...
plt.xlabel('Index')
plt.ylabel('LL')
plt.title('Predict/True value(40μm dataset)')
# 设置y轴的最大值为3.5
plt.ylim(top=3.5)
